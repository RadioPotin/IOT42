...
helm upgrade --install gitlab ./confs/gitlab 
...
argocd app create pgueugno-bonus --repo http://localhost:8080/root/bonus.git \
  --sync-policy auto \
...
# argocd app create pgueugno-bonus --repo git@localhost:8080:root/bonus.git --sync-policy auto --dest-server https://kubernetes.default.svc --dest-namespace dev --port-forward --port-forward-namespace argocd
